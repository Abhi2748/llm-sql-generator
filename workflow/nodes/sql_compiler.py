...
def compile_candidate_sql(
  ...
  LATERAL FLATTEN(input => {parent_var}:{rel_no_star}) {flatten_alias}
    ...
    def expr_for_path(path: str, cast: Optional[str]) -> str:
        ...
        best_array: Optional[str] = None
        for ap in flatten_arrays:
            _, ap_expr = normalize_path(ap)
            if expr_path == ap_expr or expr_path.startswith(ap_expr + ":"):
                if best_prefix is None or len(ap_expr) > len(best_prefix):
                    best_prefix = ap_expr
                    best_array = ap

        if best_prefix is not None and best_array is not None:
            base_alias = array_alias_map.get(best_array, base_alias)
            rel_path = (
                expr_path[len(best_prefix) + 1 :]
                if expr_path.startswith(best_prefix + ":")
                else ""
            )
        else:
            rel_path = expr_path
    ...
    for f in (query_spec.get("filters") or []):
        ...
        if op == "eq":
            where_clauses.append(f"{lhs} = {_literal_sql(val)}")
        elif op == "neq":
            where_clauses.append(f"{lhs} <> {_literal_sql(val)}")
        elif op == "gt":
            where_clauses.append(f"{lhs} > {_literal_sql(val)}")
            ...
            where_clauses.append(f"{lhs} ILIKE '%' || {_literal_sql(val)} || '%'")
        elif op == "in":
            if not isinstance(val, list):
                issues.append(f"Filter op 'in' requires a list value, got {type(val).__name__}")
                continue
            in_list = ", ".join(_literal_sql(v) for v in val)
            where_clauses.append(f"{lhs} IN ({in_list})")
        else:
    ...
    for o in (query_spec.get("order_by") or []):
        ...
        if not expr_alias or not isinstance(expr_alias, str):
            continue
        direction = (o.get("direction") or "asc").upper()
        if direction not in {"ASC", "DESC"}:
            direction = "ASC"

        if expr_alias in known_aliases:
            order_expr = expr_alias
        elif ":" in expr_alias or "[*]" in expr_alias:
            order_expr = expr_for_path(expr_alias, None)
        else:
            order_expr = expr_alias
