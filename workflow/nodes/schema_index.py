...
def _iter_nodes(obj: Any, prefix: str = "", max_array_samples: int = 200):
...
def build_schema_index_and_catalog(
    json_sample: Any,
    catalog_limit: int = 60,
    max_array_samples: int = 200,
) -> Dict[str, Any]:
    ...
    for path, value, kind in _iter_nodes(
        json_sample, prefix="", max_array_samples=max_array_samples
    ):
        if kind == "array":
            arrays.append(path)
            continue
