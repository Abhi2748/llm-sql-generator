...
class FakeLLM:
    ...
    def invoke(self, messages):
        ...
        # Plan agent is deterministic now (ADR 0002); this branch is unused but kept
        # so older prompt fingerprints don't crash if somehow invoked.
...
class DirtyIntentFakeLLM(FakeLLM):
    """Intent returns an unknown path so static_validate scores low and critic must run."""
    ...
    def _intent_response(self):
        return _Resp(
            json.dumps(
                {
                    "select": [
                        {"path": "ecommerce_events[*]:totally_unknown_field_xyz", "alias": "x", "cast": "string"},
                    ],
                    "filters": [],
                    "group_by": [],
                    "aggregations": [],
...
class TestGraphMockLLM(unittest.TestCase):
    ...
    def test_critic_skipped_when_validation_clean(self):
        with open(os.path.join("data", "sample_data.json"), "r", encoding="utf-8") as f:
            sample = json.load(f)
        ...
        validation = (result.get("state") or {}).get("validation") or {}
        ranked = result.get("ranked_candidates") or []
    ...
    def test_critic_runs_when_validation_dirty(self):
        ...
        llm = DirtyIntentFakeLLM()
        result = run_workflow(
            question="List a made-up field",
            json_sample=sample,
            table_name="customer_data",
            json_column="raw_data",
            llm=llm,
        ...
        ranked = result.get("ranked_candidates") or []
        self.assertTrue(
            (validation.get("top_score") or 0) < 90 or (ranked and (ranked[0].get("issues") or []))
        )
        self.assertEqual(llm.critic_calls, 1)
        self.assertIsNotNone(result.get("critic_notes"))
